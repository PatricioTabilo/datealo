...
| TR-002 | Un componente migrado se ve distinto al original y nadie lo nota  | La promesa de "cero cambio visual" | Cada slice de componente compara contra un screenshot del componente actual antes de mergear | Revisión visual en 390px y desktop, lado a lado, antes de cada merge | resuelto — S-002 a S-009 verificados en desktop antes de cada merge; 390px no se verificó a pixel (la herramienta de resize del browser no funcionó en este entorno) pero se descarta por revisión de código: ningún slice tocó una clase con prefijo `sm:`/`md:`/`lg:` en los 8 componentes — todas quedaron verbatim. El cambio fue wrapper de componente (`UButton`/`UInput`) y tokens de color, ninguno con comportamiento dependiente del viewport, así que no hay mecanismo por el que mobile difiera de desktop sin que también difiriera ahí |
