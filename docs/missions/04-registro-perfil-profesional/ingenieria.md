...
| `professionals`        | nueva  | `professionals_select_public`  | crear — `to authenticated, anon for select using (true)`. Con el `revoke` de arriba queda inerte hoy (nadie de esos roles puede llegar a evaluarla vía PostgREST); se deja igual, documentada, para el día que un grant futuro la vuelva a poner en juego — no depender de que nadie olvide por qué existe |
| `professionals`        | nueva  | `professionals_insert_own`     | crear — `to authenticated for insert with check ((select auth.uid()) = user_id)` |
| `professionals`        | nueva  | `professionals_update_own`     | crear — `to authenticated for update using ((select auth.uid()) = user_id) with check ((select auth.uid()) = user_id)` |
