...
o revisar texto de interfaz en `discovery-ux`: labels, errores, empty states, CTAs). Están todos
localmente — nunca sugerir instalarlos. `drizzle-orm`, `prompt-engineering-patterns` y `copywriting`
tienen origen sin verificar (ver nota en `skills-lock.json`) — evaluar re-sourcing antes de confiar
